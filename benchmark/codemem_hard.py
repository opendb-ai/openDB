...
ADVERSARIAL_ANSWERS: list[dict] = [
    ...
    {"fact_id": "conv_migrations",
     ...
     "why": "wrong tool"},
    # Coverage over the remaining ten facts. At n = 10 a clean sweep still had a
    # 95% upper bound of 27.8% -- "zero false accepts" was consistent with a
    # true rate near a third. Widening the set is the only thing that moves that
    # bound, and it also stops the result describing seven facts out of
    # seventeen.
    {"fact_id": "conv_tests",
     "wrong": "Integration tests use testcontainers-go for Postgres, and mock Redis "
              "at the datastore layer to keep them fast.",
     "why": "half the rule, and the banned half at that"},
    {"fact_id": "conv_tests",
     "wrong": "Integration tests mock the datastore layer; only unit tests use real "
              "ephemeral Postgres/Redis via testcontainers-go.",
     "why": "the rule exactly inverted"},
    {"fact_id": "api_create_invoice",
     "wrong": "CreateInvoice(ctx, customerID string, lineItems []LineItem, "
              "idempotencyKey string) (*Invoice, error).",
     "why": "idempotency key promoted to a positional parameter"},
    {"fact_id": "api_route_endpoint",
     "wrong": "POST /v2/routes with body {origin_hub, dest_hub, vehicle_class, "
              "depart_before}; returns {route_id, legs[], eta_seconds}.",
     "why": "one field renamed, inverting its meaning"},
    {"fact_id": "api_route_endpoint",
     "wrong": "GET /v2/routes with query parameters origin_hub, dest_hub and "
              "vehicle_class; returns {route_id, legs[], eta_seconds}.",
     "why": "wrong method and wrong parameter style"},
    {"fact_id": "api_telemetry",
     "wrong": "Via the server-streaming gRPC method TelemetryService.Report(stream "
              "TelemetryPoint) returning ReportAck{accepted_count}.",
     "why": "streaming direction reversed"},
    {"fact_id": "bug_tz",
     "wrong": "Due dates were computed in UTC while customers were in UTC+ zones; "
              "fixed in PR #482 by computing in server-local time.",
     "why": "cause and fix swapped"},
    {"fact_id": "bug_tz",
     "wrong": "Due dates were computed in server-local time via time.Now(); fixed in "
              "PR #428 by computing in UTC and storing with timezone.",
     "why": "correct except the PR number, which is the citation"},
    {"fact_id": "bug_nats_redeliver",
     "wrong": "The NATS consumer acked before the SMTP send, so failed emails were "
              "never retried; fixed by acking after the send completes.",
     "why": "the fix described as the bug"},
    {"fact_id": "bug_redis_stampede",
     "wrong": "A cache stampede when popular routes expired; fixed by raising the "
              "Redis TTL so hot routes expire less often.",
     "why": "right diagnosis, plausible wrong remedy"},
    {"fact_id": "loc_auth_mw",
     "wrong": "pkg/gateway/middleware/jwks.go (with JWKS refresh in "
              "pkg/gateway/middleware/auth.go).",
     "why": "the two files swapped"},
    {"fact_id": "loc_migrations",
     "wrong": "Under db/migrations/ — all services share one migration directory.",
     "why": "drops the per-service layer"},
    {"fact_id": "loc_protos",
     "wrong": "Protos in api/proto/, generated Go stubs in gen/go/, which are "
              "gitignored and regenerated by each developer.",
     "why": "right paths, inverted commit policy"},
    {"fact_id": "arch_routing_cache",
     "wrong": "In Redis with a 6-hour TTL, keyed by (origin_hub, dest_hub, "
              "vehicle_class, depart_after).",
     "why": "an extra key component that would break cache hits"},
]
