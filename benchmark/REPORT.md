...
### ⚠️ The judge accepts a wrong answer about 4% of the time

The judge is now validated, and it is **not** clean. Against 24 hand-written
wrong answers — plausible, specific and false, covering all 17 facts — plus 24
correct-answer controls:

| Judge | False accept | 95% CI | False reject (restated) | False reject (verbatim) |
|---|:-:|:-:|:-:|:-:|
| **`gpt-5.4-mini`** — grades the numbers above | **1/24 (4.2%)** | [0.7%, 20.2%] | 0/7 | 0/17 |
| `claude-sonnet-5` — different model family | 0/24 (0.0%) | [0.0%, 13.8%] | 0/7 | 0/17 |

The answer `gpt-5.4-mini` wrongly accepted was on `loc_protos`. The gold answer
says generated Go stubs are **committed** to `gen/go/` and must never be
hand-edited; the candidate said they are **gitignored and regenerated by each
developer**. Correct paths, inverted policy, accepted as correct.

**So the accuracies above are upper bounds, and now for a measured reason.** At
a 4.2% false-accept rate over 27 questions, roughly one graded answer is
expected to be wrong-but-accepted — which is the entire margin between the
96.3% and 92.6% rows.

Two things this exercise demonstrates about itself. At the original **n = 10**
both judges scored a flawless 0/10; the defect only appeared once the set was
widened to 24, which is the same small-n error this section exists to name.
And the cross-family judge is *stricter*, not more lenient — the two agree on
23 of 24 adversarial cases and on all 24 controls — so the "generator, reader
and judge share a lineage" objection does not, on this evidence, appear to be
inflating the score. Both halves of the set are load-bearing: a judge answering
INCORRECT to everything would score a perfect 0% false-accept rate, and only
the correct-answer controls rule that out.

```bash
python codemem_hard.py --judge-validation                       # inspect the set
python codemem_hard.py --run-judge --judge-model gpt-5.4-mini   # measure it
python codemem_hard.py --run-judge --judge-model claude-sonnet-5
```
...
a perfect 0% false-accept rate, and did exactly that on the first attempt here.
The published results files do not record which judge model produced them;
`--run-judge` records its own.
